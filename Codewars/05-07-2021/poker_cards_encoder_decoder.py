...
def encode(cards):
    base_values = {code: value for value, code in enumerate(card_names)}
    create_values_dict = lambda multiplier: { code: value + 13 * multiplier for code, value in base_values.items() }
    encoder_dict = {type: create_values_dict(multiplier) for multiplier, type in enumerate(['c', 'd', 'h', 's'])}
    return sorted([encoder_dict[card[1]][card[0]] for card in cards])

def decode(cards):
    cards.sort()
    symbols = [name + type for type in ['c', 'd', 'h', 's'] for name in card_names ]
    decoder_dict = { value: code for value, code in enumerate(symbols) }
    return [decoder_dict[card] for card in cards]
